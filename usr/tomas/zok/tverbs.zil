...
<ROUTINE TELL-SCORE ()
	 ...
	 <COND (<NOT <EQUAL? ,MOVES 1>>
		<TELL "s">)>
	 <TELL
", you have achieved a score of, um, oh, call it " N ,SCORE " out of 400
points. This gives you the rank of moron">
	 ;<COND (,MALE
...
<ROUTINE CORP-NOTICE (STRING)
	 <TELL
"Here " .STRING " a transcript of interaction with " ,SNR ,PERIOD-CR>>

<ROUTINE V-DIAGNOSE ()
	 <TELL "You are in good health">
	 <TELL ,PERIOD-CR>>

<ROUTINE NO-AILMENTS ("OPTIONAL" (EH <>))
      	 <COND (<NOT .EH>
		<CRLF>)>
	 <COND (<NOT ,SECOND-HALF>
	        <TELL "(You don't have the sniffles, a
sore throat, a fever, or even aches and pains.)" CR>)
	       (.EH
...
<ROUTINE V-GET-UNDRESSED ()
	 <TELL 
"You look around, but can't seem to locate a changing room." CR>
	 ;<COND (<PRSO? ,ROOMS>
...
<ROUTINE V-GIVE ()
	 <COND (<FSET? ,PRSI ,ACTORBIT>
		<COND (<EQUAL? ,PRSI ,CONTRA>
		       <TELL "Rudely, in a jeering voice,">)
		      (T
		       <TELL "Briskly,">)>
		<TELL  T ,PRSI " refuses your offer." CR>)
	       (T
...
<ROUTINE V-PLAY ()
	 <COND (<EQUAL? ,HERE ,BEDROOM>
		<TELL
"You'll have to use the stereo controls." CR>)
	       (T
		<YUKS>)>>
...
<ROUTINE V-STAND ()
	 <COND (<EQUAL? ,HERE ,IN-BED>
		<GET-OUT-OF-BED <>>)
	       (<EQUAL? ,P-PRSA-WORD ,W?HOLD> ;"for HOLD UP OBJECT"
		<WASTES>)
...
,YOU-CANT "pick up anything while using " 'HANDS "s to cover" TR ,HAND-COVER>)
		...
		<RTRUE>)
	       (<AND <FSET? ,PRSO ,TRYTAKEBIT>
		     <IN? ,PRSO ,CONTRA>>
		<TELL
"The " D ,CONTRA " snatches it back. \"Hey, slobola!! This is MINE!\"" CR>
		<RTRUE>)
	       (<NOT ,PRSI>
		<RFALSE>)
	       (<IN? ,PRSO ,PRSI>
	 ...
	 <COND (<PRSO? ,ROOMS>
		<COND ;(<EQUAL? ,P-PRSA-WORD ,W?GET>
		      ...
		      (T
		       ...
		       <YUKS> ;"IS THIS THE RIGHT RESPONSE? -- TJB"
		       <RTRUE>)>)
	       (<FSET? ,PRSO ,WORNBIT>
		<FCLEAR ,PRSO ,WORNBIT>
...
<ROUTINE V-WAIT ("OPTIONAL" (NUM 1))  ;"SWITCHED TO 1"
	 <TELL "Time passes..." CR>
	 <REPEAT ()
...
<ROUTINE DESCRIBE-OBJECTS ("AUX" O STR (1ST? T) (AV <LOC ,WINNER>))
	 ...
	 <REPEAT () ;"FDESCS and MISC."
		 <COND (<NOT .O>
			...
			<TELL ;"   " .STR>  ;"LETS HACK THIS SHIT - TJB"
			<COND (<FSET? .O ,CONTBIT>
...
<ROUTINE GOTO (NEW-LOC "OPTIONAL" (DONT-DESCRIBE-SIDEKICK <>) "AUX" OLD-HERE)
	 ...
	 <COND (,CONTRA-FOLLOW
		<COND (<EQUAL? ,HERE ,BARE-HALL>
		      ...
		      (T 
		       <TELL
"Keeping his distance," T ,CONTRA " follows you grumpily." CR>
		<MOVE ,CONTRA ,HERE>)>)>
