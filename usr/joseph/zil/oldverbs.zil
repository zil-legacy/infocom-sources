...
<ROUTINE DESCRIBE-ROOM ("OPTIONAL" (LOOK? <>) "AUX" V? STR L)
	 ...
	 <COND (<NOT ,LIT>
		...
		<TELL "Light comes from the next room." CR>)>
...
<ROUTINE ALREADY (OBJ "OPTIONAL" (STR <>))
	<SETG CLOCK-WAIT T>
	<TELL !\(>
	<COND ;(<NOUN-USED? .OBJ ,W?VIDEOTAPE>	;"confusing in secret passage"
	       <TELL "The videotape">)
	      (T <TELL The .OBJ>)>
...
;<ROUTINE V-BRUSH ()
	 <COND (<NOT ,PRSI>
		<COND (<HELD? ,VIDEOTAPE>
		       <TELL "(with the " D ,VIDEOTAPE ")" CR>
		       <PERFORM ,V?BRUSH ,PRSO ,VIDEOTAPE>
		       <RTRUE>)
		      (T
		...
		<TELL "With " a ,PRSI "!" CR>)
	       (<NOT <DOBJ? TEETH>>
		<TELL
"In general, " D ,VIDEOTAPE "es are meant for teeth." CR>)
	       (T
...
<ROUTINE V-THROUGH ("AUX" RM DIR)
	<COND (<AND <NOUN-USED? ,PRSO ,W?WINDOW>
		    <FSET? ,PRSO ,DOORBIT>
...
<ROUTINE V-EXAMINE ("AUX" (TXT <>))
	 <COND (<OR <==? ,PRSO ,PSEUDO-OBJECT>
		    <AND <NOUN-USED? ,PRSO ,W?VIDEOTAPE ;,W?TAPE ;,W?KEYHOLE>
			 <GLOBAL-IN? ,PRSO ,HERE>>>
		<SET TXT T>)>
	 <COND (<DOBJ? INTDIR>
		<SETG CLOCK-WAIT T>
...
<ROUTINE HELP-TEXT ()
	<SETG CLOCK-WAIT T>
	<TELL
"[You'll find plenty of help in your package.|
If you're really stuck, you can EAT SHIT AND DIE SUCKER!]">>
...
;<ROUTINE V-KISS ()
	 <TELL "family entertainment, not a video nasty." CR>>

<ROUTINE V-KISS ("AUX" X)
	 <COND (<EQUAL? ,PRSO ,PLAYER>
		<TELL "You kiss " ,PLAYER " for a minute. Hmm!" CR>)
	       (<AND <FSET? ,PRSO ,PERSONBIT>
		     <NOT <FSET? ,PRSO ,MUNGBIT>>>
		<FACE-RED>)
 ...
 <COND (<OR <FSET? ,PRSO ,DOORBIT>
	...
	<COND (<FSET? ,PRSO ,OPENBIT>
	      ...
	      (<AND <SET P <DOOR-ROOM ,HERE ,PRSO>>
	       ...
	       <UNSNOOZE .P>
	       <THIS-IS-IT .P>
	       <TELL  " You knock on the ,PRSO and look out the window.">)>)
...
<ROUTINE V-LOOK-INSIDE ("OPTIONAL" (DIR ,P?IN) "AUX" RM)
	...
	;	       <COND (<OR <FSET? <SET RM ,P-IT-OBJECT> ,CONTBIT>
	 ...
	 <COND (<DOBJ? GLOBAL-HERE>
		...
		<RTRUE>)
	       ;(<AND <IN? ,PRSO ,ROOMS>	;<FSET? ,PRSO ,RLANDBIT>
		     <NOT <NOUN-USED? ,PRSO ,W?VIDEOTAPE>>
...
<ROUTINE V-LOOK-THROUGH ("OPTIONAL" (INSIDE <>) "AUX" RM)
	 <COND (<FSET? ,PRSO ,DOORBIT>
		...
		<TELL "You forgot to bring your X-ray glasses." CR>)
	       (.INSIDE <RFALSE>)
	       (<FSET? ,PRSO ,TRANSBIT>
		<TELL "Everything looks browner." CR>)
...
<ROUTINE PRE-TAKE-WITH ("AUX" X)
	 <COND (<DOBJ? YOU>
		...
		<YOU-CANT "reach">)
	       (<AND <IN? ,WINNER ,PRSO>
		     <NOT <NOUN-USED? ,PRSO ,W?VIDEOTAPE ;,W?KEYHOLE>>>
		<SETG CLOCK-WAIT T>
		<TELL
...
<ROUTINE V-YES ("OPTIONAL" (NO? <Y>) "AUX" PER)
