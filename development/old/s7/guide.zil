...
"blinking wildly, indicating a spaceship in the vicinity">)
		      (T
		       <TELL "dark">)>
		<TELL "dark. It has two small buttons, a red one
labelled \"Call Engineer\" and a green one labelled \"Hitchhike.\"">
		<FINE-PRODUCT>
		<CRLF>
		;<TELL
" Affixed to the Thumb is a lifetime " D ,GUARANTEE "." CR>)
		...
		<TELL "Impossible." CR>)
	       (T
		<TELL "[This object is only here for show, right now.]" CR>)>>
...
<ROUTINE RED-BUTTON-F ()
	 <TELL "[The red button has been temporarily suspended.]">
	 ;<COND (<VERB? PUSH>
		<COND (<NOT <HELD? ,THUMB>>
...
and leaves again." CR>)
		      (T
		       ...
		       <TELL
"With a screech of ion brakes a " ,SCC-STR " Repair Robot pulls up on a bike
...
<ROUTINE GREEN-BUTTON-F ()
	 <TELL "[The green button has been temporarily suspended.]">
	 ;<COND (<VERB? PUSH>
		<COND (<NOT <HELD? ,THUMB>>
...
by trained " ,SCC-STR " Field Engineers." CR>)>>
...
D ,GUIDE " is a wholly remarkable product." ,ALREADY-KNOW-THAT CR>)
		      ;(<EQUAL? ,PRSI ,DANGLY-BIT ,SMALL-RECEPTACLE ,LARGE-PLUG
			      ,SMALL-PLUG ,GREEN-BUTTON ,RED-BUTTON>
		       <TELL "Part of">
		       <COND (<AND <EQUAL? ,PRSI ,LARGE-PLUG>
				   <NOT <IN? ,LARGE-PLUG ,SPARE-DRIVE>>>
			      <ARTICLE ,SPARE-DRIVE>)
			     (T
			      <ARTICLE <LOC ,PRSI>>)>
		       <TELL "." CR>)
		      ;(<EQUAL? ,PRSI ,WAR-CHAMBER>
		       <TELL
...
alternative to a " D ,POETRY " reading." CR>)
		      ;(<EQUAL? ,PRSI ,VOGON-CAPTAIN ,FLEET>
		       <TELL
...
liquid is a good source." CR>)
		      ;(<EQUAL? ,PRSI ,GALLEY>
		       <TELL
"The " ,SCC-STR " incompetently produces a wide range of inefficient and unreliable
...
intelligence. The most popular definition appears in the " ,SCC-STR " android
...
estate agent waive his fee.\"" CR>)
		      ;(<EQUAL? ,PRSI ,SCREENING-DOOR>
		       <TELL
"These " ,SCC-STR " doors screen visitors for such qualities as intelligence
and ability to time travel" ,ALSO-SEE ,SCC-STR ", " ,GPP ", Intelligence,
and Time Travel." CR>)
		      (<EQUAL? ,PRSI ,MARVIN>   ;<,HATCHWAY ,EDDIE>
		       <TELL
,GPP " are a misguided attempt by the " ,SCC-STR " to make their machines behave
...
unless you replace it immediately." CR>)
		      ;(<EQUAL? ,PRSI ,NUTRIMAT ,NUT-COM-INTERFACE>
		       <TELL
"A typically unreliable " ,SCC-STR " product, the " D ,NUTRIMAT " analyses
...
is a good thing to go for.">
		       <CRLF>)
		      ;(<EQUAL? ,PRSI ,POCKET-FLUFF ,JACKET-FLUFF
...
one's ear, allows one to understand any language." CR>)
		      (<EQUAL? ,PRSI ,TOWEL>
		       <TELL "A towel is the most useful thing (besides
the Guide) a Galactic hitchhiker can have. Its uses include travel,
combat, communications, protection from the elements, hand-drying and
reassurance. Towels have great symbolic value, with many associated
points of honour. Never mock the towel of another, even if it has little
pink and blue flowers on it. Never do something to somebody else's towel
that you would not want them to do to yours. And, if you borrow the
towel of another, you MUST return it before leaving their world." CR>)
		      ;(<EQUAL? ,PRSI ,THUMB ,GUARANTEE>
		       <TELL
...
,SCC-STR " lifetime " D ,GUARANTEE "s." CR>)
...
will tell you." CR>)
		      ;(<EQUAL? ,PRSI ,BEAST>
		       <TELL "The " D ,BEAST " is a mind-bogglingly
stupid animal. It has almost no capacity for learning from experience
and is therefore surprised by virtually everything that happens to it.
Here is an example of how stupid it is: it thinks that if you can't see
it, it can't see you.|
Its behaviour would be quite endearing if it wasn't spoilt by this one thing:
it is the most violently carnivorous creature in the Galaxy. Avoid, avoid,
avoid." CR>)
		      ;(<EQUAL? ,PRSI ,FRONT-PORCH>
		       <TELL
...
a slice of lemon wrapped around a large gold brick." CR>)
		      (<EQUAL? ,PRSI ,EARTH>     ;<EQUAL? ,PRSI ,THIRD-PLANET ,MAZE>
		       <TELL "Mostly harmless." CR>)
		      ;(<EQUAL? ,PRSI ,COUNTRY-LANE>
		       <TELL ,SPACE-TEXT CR>)
		      ;(<FSET? ,PRSI ,TOOLBIT>
		       <TELL
...
or death one, in which case, sure, go ahead, panic." CR>)
		      ;(<EQUAL? ,PRSI ,BEDROOM ,RIFLES ,BLASTER ,TEA ,NO-TEA
			      ,BEAST-GUN ,SUBSTITUTE ,AFT-CORRIDOR
			      ,AIRLOCK ,INSIDE-WHALE>
		       <TELL
...
<CONSTANT SCC-STR "Sirius Cybernetics Corporation">
