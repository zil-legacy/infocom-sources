...
notice, of course; just don't say you weren't warned.")
	(SOUTH TO AIRLOCK IF HATCH IS OPEN)
	(UP TO AIRLOCK IF HATCH IS OPEN)
	(ADJACENT <TABLE (LENGTH BYTE) AIRLOCK T>)
	(NORTH TO BLIGHTED-GROUND)
...
<ROOM AIRLOCK
      ...
      (ADJACENT <TABLE (LENGTH BYTE) RAMP T>)
...
<OBJECT HATCH
	(LOC LOCAL-GLOBALS)
	(DESC "hatch")
	(SYNONYM HATCH ;AIRLOCK)
	(FLAGS DOORBIT OPENBIT)
	(ACTION HATCH-F)>

<ROUTINE HATCH-F ()
	 <COND (<AND <NOT <EQUAL? ,HERE ,AIRLOCK ,RAMP>>
...
the south.
[Exits: east, west, southwest, south.]")
	(SOUTH TO RAMP)
	;(NORTH PER VEHICLE-ROUTINE-YET-TO-BE-WRITTEN)
	(EAST TO FOOTHILLS)
	(WEST TO BOTTOM-MOUNTAIN)
	(SW TO TUNNEL)
	(DOWN TO TUNNEL)
	;(NORTH PER WANDER-AROUND)
...
<ROOM FOOTHILLS
	(LOC ROOMS)
	(DESC "Foothills")
	(LDESC "Foothills. Exit west.")
	(WEST TO BLIGHTED-GROUND)
	(FLAGS ONLANDBIT ONBIT ;COLDBIT ;NOAIRBIT)>

<ROOM BOTTOM-MOUNTAIN
	(LOC ROOMS)
	(DESC "Bottom of Mountain")
	(LDESC "Bottom of mountain. Exit east.")
	(EAST TO BLIGHTED-GROUND)
	(FLAGS ONLANDBIT ONBIT ;COLDBIT ;NOAIRBIT)>

<ROOM TUNNEL
	(LOC ROOMS)
	(DESC "Tunnel")
	(LDESC "Tunnel. Exits: northeast, west.")
	(NE TO BLIGHTED-GROUND)
	(WEST TO STORAGE-AREA)
	(FLAGS ONLANDBIT ONBIT ;COLDBIT ;NOAIRBIT)>

<ROOM STORAGE-AREA
	(LOC ROOMS)
	(DESC "Storage Area")
	(LDESC "Storage area. Exits: east, north.")
	(EAST TO TUNNEL)
	(NORTH SORRY
"There will be an elevator there someday, but not yet. Sorry.")
	(FLAGS ONLANDBIT ONBIT ;COLDBIT ;NOAIRBIT)>

;<ROUTINE I-NEW-SOFTWARE ()
	 >
