...
<CONSTANT FUMBLE-NUMBER:NUMBER 7>
<CONSTANT FUMBLE-PROB:NUMBER 8>
...
<ROUTINE CHECK-DOOR (DR)
	...
	<COND (<FSET? .DR ,OPENBIT> <TELL "open">)
	      (T
	       <TELL "closed and ">
	       <COND (<NOT <FSET? .DR ,LOCKEDBIT>> <TELL "un">)>
	       <TELL "locked">)>
	<TELL "." CR>>
...
<ROUTINE OKAY ("OPTIONAL" (OBJ <>) (STR <>))
	...
	<COND (.OBJ
	       ...
	       <COND (<=? .STR "on">		<FSET .OBJ ,ONBIT>)
		     ...
		     (<=? .STR "locked">	<FSET .OBJ ,LOCKEDBIT>)
		     (<=? .STR "unlocked">	<FCLEAR .OBJ ,LOCKEDBIT>)>)>
	<COND (<OR .STR <NOT .OBJ>>
...
<ROUTINE VISIBLE? ;"can player SEE object?"
	 ...
	 <COND (<AND <SET L <GETP ,HERE ,P?ADJACENT>>
		     <SET L <INTBL? <META-LOC .OBJ> <REST .L 1> <GETB .L 0> 1>>
		     <T? <GETB .L 1>>>
		<RTRUE>)>
...
<ROUTINE PRE-THROUGH ("AUX" VEH)
 <COND (<EQUAL? ,PRSO ,ROOMS ,GLOBAL-HERE>
...
<ROUTINE V-EXAMINE ("AUX" (TXT <>))
	 ...
	 <COND (<EQUAL? ,PRSO ,INTDIR>
		...
		<TELL "(If you want to see what's there, go there!)" CR>)
	       (<EQUAL? ,PRSO ,HANDS ,HEAD ,EYES ,TEETH ,EARS ,WALL ,LIGHT-GLOBAL>
		<NOTHING-SPECIAL>)
	       ;(<DOBJ? NOW-WEARING>
...
<ROUTINE V-LOOK-UNDER ()
	 <COND (<EQUAL? ,PRSO ,EYES ,HANDS ,HEAD ,EARS ,TEETH>
		<WONT-HELP>)
	       (<HELD? ,PRSO>
		<TELL "You're ">
...
<ROUTINE PRE-PUT ()
	 ...
	 <FCLEAR ,PRSO ,WORNBIT>
	 <COND (<EQUAL? ,PRSO ,HEAD ,HANDS>
		<WONT-HELP>
		<RTRUE>)
	       (<IN? ,PRSO ,GLOBAL-OBJECTS>
		<NOT-HERE ,PRSO>
		<RTRUE>)
	       (<EQUAL? ,PRSI ,FLOOR ,GLOBAL-HERE <> ;,POCKET>
		<RFALSE>)
	       (<AND ;<T? ,PRSI>
...
<ROUTINE V-SIT ("OPTIONAL" (LIE? <>))
 <COND (<AND <==? ,WINNER ,PLAYER>
	     <OR <FSET? ,PRSO ,VEHBIT>
		 <AND <EQUAL? ,PRSO ,GLOBAL-HERE ,HERE ,FLOOR>
...
<ROUTINE V-SLAP ()
 <COND (<EQUAL? ,PRSI ,ROOMS> <SETG PRSI <>>)>
...
<ROUTINE PRE-TAKE ("AUX" L)
	 <COND (<EQUAL? ,PRSO ;,NOW-WEARING ,FLOOR ,WALL ;,KEYHOLE>
		<HAR-HAR>)
	       (<EQUAL? ,PRSO ,HANDS ,YOU>
		<RFALSE>)
...
<ROUTINE V-DISEMBARK ()
	 <COND (<ROOM-CHECK>
		<RTRUE>)
	       (<EQUAL? ,PRSO ,ROOMS ,HERE ,GLOBAL-HERE ;,GLOBAL-WATER>
		<COND (<AND <==? ,WINNER ,PLAYER>
			    <NOT <IN? ,PLAYER ,HERE>>
			    ;<T? ,PLAYER-SEATED>>
		       <OWN-FEET>)
...
<ROUTINE V-WAIT-FOR ("AUX" WHO)
	 <COND (<AND <NOT <==? -1 ,P-NUMBER>>
		     <EQUAL? ,PRSO ,ROOMS ,TURN ,INTNUM>>
		<COND ;(<T? ,P-TIME>
		       <V-WAIT-UNTIL>)
		      (T <V-WAIT ,P-NUMBER>)>)
	       (<EQUAL? ,PRSO ,ROOMS ,TURN ,GLOBAL-HERE>
		<V-WAIT>)
	       (<EQUAL? ,PRSO ,PLAYER>
		<ALREADY ,PLAYER "here">)
...
<ROUTINE OPEN-DOOR-AND-CLOSE-IT-AGAIN (OBJ)
	...
	<COND (<NOT <==? ,WINNER ,PLAYER>>
	       <RTRUE>)>
	<TELL "(You ">
	<COND (<FSET? .OBJ ,LOCKEDBIT>
	       <FCLEAR .OBJ ,LOCKEDBIT>
	       <TELL "unlock and ">)>
	<TELL "open the ">
