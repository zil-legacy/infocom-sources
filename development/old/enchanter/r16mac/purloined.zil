...
<ROUTINE MOVE-ADVENTURER (RM "AUX" L (CNT 0) OBJ)
	 <SET L <LOC ,ADVENTURER>>
...
<ROUTINE DOOR-FROM? (HERE THERE "AUX" P L T)
	 <SET P 0>
	 <REPEAT ()
...
<ROUTINE ADVENTURER-DECIDE (L "AUX" P S T)
	 <SET P <NEXTP .L 0>>
	 <PUT ,ADV-POSS 0 0>
	 <REPEAT ()
		 <COND (<L? .P ,LOW-DIRECTION> <RETURN>)
		       (T
			<SET S <PTSIZE <SET T <GETPT .L .P>>>>
			<COND (<OR <EQUAL? .S ,UEXIT>
			       	   <AND <EQUAL? .S ,DEXIT>
					<NOT <FSET? <GETB .T ,DEXITOBJ>
						    ,LOCKEDBIT>>
			       ...
			       <PUT ,ADV-POSS
				    0
				    <SET T <+ <GET ,ADV-POSS 0> 1>>>
			       <PUT ,ADV-POSS .T .S>)>)>
		 <SET P <NEXTP .L .P>>>
	 <COND (<EQUAL? <SET S <GET ,ADV-POSS 0>> 0>
		<TELL "**BUG: Can't move." CR>)
	       (<EQUAL? .S 1>
...
<ROUTINE PURLOINED-ROOM-A ("AUX" T)
	 <SET T <TREASURE-HERE? ,PURLOINED-ROOM>>
	 <COND (.T
		<SETG ADVENTURER-STAY T>
		<COND (<EQUAL? ,HERE ,PURLOINED-ROOM>
		       ...
		       <COND (<FSET? ,DOOR-REALITY ,INVISIBLE>
		              <TELL
"The adventurer stops to pick up the " D .T ". As he rises, he casts a cursory
glance at the door, then he notices you. ">
			      <COND (,ADVENTURER-CHARMED
				     <TELL
"He smiles at you as he pockets the " D .T "." CR>)
				    (T
				     <TELL
...
Wooden door? You rub your eyes for a moment and look again. Yes, just a
plain wooden door." CR>
