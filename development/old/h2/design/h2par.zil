...
<ROUTINE FAKE-ORPHAN ("AUX" TMP)
	 ...
	 <COND ;(<VERB? WALK WALK-TO>
		<TELL "ere">)
	       (<EQUAL? <GETB ,P-SYNTAX ,P-SFWIM1> ,PERSONBIT>
		<TELL "om">)
	       (T <TELL "at">)>
	 <TELL " do">
	 <COND (<T? ,DONT-FLAG>
...
"[Your command was not complete. Next time, type wh">)>
	       <COND (<EQUAL? .VERB ,ACT?HEAD ;WALK>
		      <TELL "ere">)
		     (<OR <AND <T? .DRIVE1>
			       <EQUAL? <GETB .DRIVE1 ,P-SFWIM1> ,PERSONBIT>>
			  <AND <T? .DRIVE2>
			       <EQUAL? <GETB .DRIVE2 ,P-SFWIM2> ,PERSONBIT>>>
		      <TELL "om">)
		     (T <TELL "at">)>
...
<ROUTINE ZMEMQB (ITM TBL "OPTIONAL" (SIZE -1) "AUX" (CNT 1)) 
	...
	<REPEAT ()
		<COND (<EQUAL? .ITM <GETB .TBL .CNT>>
		      ...
		      (<IGRTR? CNT .SIZE> <RFALSE>)>>>  

<GLOBAL LIT:OBJECT DRIVEWAY>	"source of light, 0=dark"
