...
<DEFINE PRINT-LEXV ("OPT" (QUIET 0)
	...
	<COND (<F? .QUIET>
	      ...
	      ;(T
	       <IFFLAG (P-DEBUGGING-PARSER <PRINT "[Debugging info: ">)
		       (T T)>)>
	;<BUFFER-PRINT .X <+ .X <* ,P-WORDLEN ,P-LEN>>>
	<REPEAT (WD (OWD 0) (IN-QUOTE <>))
		<SET WD <ZGET .X 0>>
		<COND (<EQUAL? .WD
   ...
   <COND (<OR <T? ,P-WON> <1? <RANDOM 2>>> ;<NOT <IGRTR? NUM 3>>
	  ...
	  <REPEAT (N (M <GETB .SYN 0>))
		...
		<COND (<AND <G=? .N ,W?A>
			    ...
			    <NOT <EQUAL? .N ,W?END.OF.INPUT ,W?INT.NUM
					    ,W?INT.TIM ,W?NO.WORD>>>
		       <PRINT-VOCAB-WORD .N>
		       <RETURN>)>>
	  <TELL "\"|">)>
...
<DEFINE CANT-FIND-OBJECT (NP PART ;SEARCH "AUX" OTHER)
 <COND (<ZERO? <NP-QUANT .NP>>	;<EQUAL? .NP ,ORPHAN-NP>
	<NP-CANT-SEE .NP>)
       (T
	<TELL "[There isn't anything to ">
	<COND (<SET OTHER <PARSE-VERB ,PARSE-RESULT>>
	       <PRINT-VOCAB-WORD .OTHER>
	       ;<SET OTHER <PARSE-PARTICLE1 ,PARSE-RESULT>>
	       <COND (<NOT <EQUAL? .PART ;.OTHER 0 1>>
		      <TELL C !\ >
		      <PRINT-VOCAB-WORD .OTHER>)>)
	      (T <TELL "do that to">)>
	<TELL "!]" CR>)>>
...
<DEFINE NP-CANT-SEE ("OPT" (NP <GET-NP>) "AUX" OTHER)
	<COND (<SET OTHER <NP-NAME .NP>>
	       <TELL "[">
	       <TELL-CTHE ,WINNER>
	       <TELL " can't see ">
	       <COND (<NOT <CAPITAL-NOUN? .OTHER>>
		      <TELL "any ">
		      <NP-PRINT .NP>)
		     (T
		      <NP-PRINT .NP T>)>
	       <TELL !\ >
	       <COND (<AND <SET OTHER <NP-LOC .NP>>
			   <OR <AND ;<EQUAL? .NP ,ORPHAN-NP>
				    ;"removed for HIT MAN ON HEAD WITH ROCK"
				    <PMEM-TYPE? .OTHER NOUN-PHRASE>
				    <TELL "in">>
			       <AND <PMEM-TYPE? .OTHER LOCATION>
				    <SET OTHER <LOCATION-OBJECT .OTHER>>
				    <PRINT-VOCAB-WORD<LOCATION-PREP .OTHER>>>>>
		      <TELL " ">
		      <TELL-THE <NOUN-PHRASE-OBJ1 .OTHER>>)
		     (T
...
<DEFINE SETUP-ORPHAN-NP (STR OBJ1 OBJ2 "OPT" (OBJ3 <>) "AUX" (NUM 2)
	...
	<COND (<INTBL? 0 <ZREST ,O-LEXV <* 2 ,P-LEXSTART>>
	       ...
	       <>)
	      (T
	       <SETG P-OFLAG <- <* ,P-LEXELEN <GETB ,O-LEXV ,P-LEXWORDS>> 1>>
...
<DEFINE INSERT-ADJS (E "AUX" CT (PTR ,P-OFLAG))
 <COND (<NOT <EQUAL? .E <> T>>
