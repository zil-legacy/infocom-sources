...
<ROOM WEST-SHORE
      ...
      (MAP-LOC <PTABLE LAKE-AREA-MAP-NUM MAP-GEN-Y-5 MAP-GEN-X-3>)
      (ICON WEST-SHORE-ICON)
      (ACTION WEST-SHORE-F)>

<ROUTINE WEST-SHORE-F (RARG)
...
<ROOM NORTH-SHORE
      ...
      (NORTH TO FROBOZZ-PHILHARMONIC-HALL)
      (NE TO THEATRE)
      (FLAGS RLANDBIT ONBIT UNDERGROUNDBIT)
      (GLOBAL LAKE-FLATHEAD)
      (MAP-LOC <PTABLE LAKE-AREA-MAP-NUM MAP-GEN-Y-4 MAP-GEN-X-4>)
      (ICON NORTH-SHORE-ICON)
      (ACTION NORTH-SHORE-F)>

<ROUTINE NORTH-SHORE-F (RARG)
	 <COND (<EQUAL? .RARG ,M-LOOK>
		<TELL
...
<ROOM EAST-SHORE
      ...
      (FLAGS RLANDBIT ONBIT UNDERGROUNDBIT)
      (GLOBAL LAKE-FLATHEAD)
      (MAP-LOC <PTABLE LAKE-AREA-MAP-NUM MAP-GEN-Y-5 MAP-GEN-X-5>)
      (ICON EAST-SHORE-ICON)
      (ACTION EAST-SHORE-F)>

<ROUTINE EAST-SHORE-F (RARG)
	 <COND (<EQUAL? .RARG ,M-LOOK>
		<TELL
...
<ROOM SOUTH-SHORE
	...
	(FLAGS RLANDBIT ONBIT UNDERGROUNDBIT)
	(GLOBAL LAKE-FLATHEAD)
	(MAP-LOC <PTABLE LAKE-AREA-MAP-NUM MAP-GEN-Y-6 MAP-GEN-X-4>)
	(ICON SOUTH-SHORE-ICON)
	(ACTION SOUTH-SHORE-F)>

<ROUTINE SOUTH-SHORE-F (RARG)
	 <COND (<EQUAL? .RARG ,M-LOOK>
		<TELL
...
"The royal yacht sits by the dock, bobbing gently in the swell of the lake.")
	...
	(ADJECTIVE ROYAL)
	(CAPACITY 200)
	(FLAGS VEHBIT DROPBIT OPENBIT SEARCHBIT CONTBIT)
	(ACTION YACHT-F)>
		      ...
		      (T
		       <MOVE ,PROTAGONIST
			     <COND (<EQUAL? ,HERE ,WEST-SHORE> ,WEST-DOCK)
				   ...
				   (T ,SOUTH-DOCK)>>
		       <SETG OLD-HERE <>>
		       <TELL "You step off the boat." CR CR>
		       <V-LOOK>)>)
	       (<VERB? SINK>
...
upward.")
      ...
      (IN PER BATHYSPHERE-ENTER-F)
      (FLAGS RLANDBIT UNDERGROUNDBIT)
      (SYNONYM HOLD)
      (MAP-LOC <PTABLE LAKE-AREA-MAP-NUM MAP-GEN-Y-5 MAP-GEN-X-1>)
      (ICON HOLD-ICON)>

<ROUTINE YACHT-ENTER-F ()
	 <GOTO ,YACHT>
	 <RFALSE>>

<ROUTINE BATHYSPHERE-ENTER-F ()
	 <COND (<FSET? ,BATHYSPHERE ,OPENBIT>
		<PERFORM ,V?ENTER ,BATHYSPHERE>)
	       (T
		<THIS-IS-IT ,BATHYSPHERE>
...
<OBJECT BATHYSPHERE
	...
	(ACTION BATHYSPHERE-F)>

<GLOBAL BATHYSPHERE-CONTROLS-DESCRIBED <>>

<ROUTINE BATHYSPHERE-F ("OPTIONAL" (VARG <>))
	 <COND (<AND .VARG
		     <NOT <EQUAL? .VARG ,M-ENTER>>>
		<RFALSE>)
	       (.VARG
		<COND (,BATHYSPHERE-CONTROLS-DESCRIBED
		       <RFALSE>)
		      (T
		       <SETG BATHYSPHERE-CONTROLS-DESCRIBED T>
		       <TELL " ">
		       <PERFORM ,V?EXAMINE ,BATHYSPHERE>
		       <RTRUE>)>)
	       (<VERB? EXAMINE>
		<COND (<IN? ,PROTAGONIST ,BATHYSPHERE>
		       <TELL "This is a cramped diving bell. The door is ">
		       <OPEN-CLOSED ,BATHYSPHERE>
		       <TELL
...
". Mounted on the outside of it is a claw-like waldo." CR>)>)
	       (<AND <VERB? ENTER>
		     <NOT <IN? ,PROTAGONIST ,BATHYSPHERE>>
		     <NOT <FSET? ,BATHYSPHERE ,OPENBIT>>>
		<DO-WALK ,P?IN>)
	       (<AND <VERB? EXIT>
		     <IN? ,PROTAGONIST ,BATHYSPHERE>>
		<COND (,HAND-IN-WALDO
...
<ROUTINE GENERIC-BATHYSPHERE-HOLE-F (TBL LEN)
	 ,HAND-HOLE>
...
<OBJECT PORTHOLE
	...
	(ADJECTIVE PORT)
	(FLAGS NDESCBIT)
	(GENERIC GENERIC-BATHYSPHERE-HOLE-F)
	(ACTION PORTHOLE-F)>
...
<ROUTINE RUBY-F ()
	 <COND (<VERB? EXAMINE>
		<TELL
"This ruby must surely be the largest jewel in the land." CR>)>>
...
<ROUTINE HAND-HOLE-F ()
	 <COND (<NOT <IN? ,PROTAGONIST ,BATHYSPHERE>>
...
"There's already" A <FIRST? ,WALDO> " in the waldo." CR>)
	       (<AND <EQUAL? ,HERE ,LAKE-BOTTOM>
		     <NOT <ULTIMATELY-IN? ,SQUID-REPELLENT ,LAKE-BOTTOM>>>
		<TELL
...
porthole, and by the time the view clears, the squid is gone and" T ,PRSO
" is lying right where it was before." CR>)
	       (<FSET? ,PRSO ,TAKEBIT>
		<MOVE ,PRSO ,WALDO>
		<FSET ,PRSO ,TOUCHBIT>
		<TELL "You pick up" T ,PRSO " in the waldo." CR>)
	       (<PRSO? ,JESTER ,JESTER-HAT ,JESTER-POCKET ,JESTER-SHOE>
		<TELL
...
<OBJECT UP-DOWN-LEVER
	(LOC BATHYSPHERE)
	(DESC "up-down lever")
	(SYNONYM LEVER)
	(ADJECTIVE UP-DOWN)
	(FLAGS NDESCBIT VOWELBIT)
	(ACTION UP-DOWN-LEVER-F)>

<GLOBAL BATHYSPHERE-DIRECTION 0> ;"1 = ascending, -1 = descending"

<GLOBAL BATHYSPHERE-DEPTH 0>

<GLOBAL CHESS-PIECE-DROWNED 0>

<ROUTINE UP-DOWN-LEVER-F ("AUX" X)
	 <COND (<VERB? EXAMINE>
		<TELL "The lever is in the ">
		<COND (<EQUAL? ,BATHYSPHERE-DIRECTION 1>
		       <TELL "up">)
		      (<EQUAL? ,BATHYSPHERE-DIRECTION -1>
		       <TELL "down">)
		      (T
		       <TELL "neutral">)>
		<TELL " position">
		<COND (<EQUAL? ,BATHYSPHERE-DIRECTION 0>
		...
		<COND (<EQUAL? ,BATHYSPHERE-DIRECTION 1>
		      ...
		      (<EQUAL? ,BATHYSPHERE-DEPTH 0>
		       <TELL ,NOTHING-HAPPENS>)
		      (T
	 	       <SETG BATHYSPHERE-DIRECTION 1>
		       <QUEUE I-BATHYSPHERE 2>
		       <TELL "The bathysphere ascends.">
		       <COND (<EQUAL? ,HERE ,LAKE-BOTTOM>
			      <MOVE ,BATHYSPHERE ,UNDERWATER>
...
<ROUTINE GLANCE-OUT-PORTHOLE ()
	 <TELL " You glance out the porthole" ,ELLIPSIS>
	 <GOTO ,BATHYSPHERE>>

<ROUTINE I-BATHYSPHERE ()
	 <QUEUE I-BATHYSPHERE -1>
	 <SETG BATHYSPHERE-DEPTH <- ,BATHYSPHERE-DEPTH ,BATHYSPHERE-DIRECTION>>
	 <COND (<IN? ,PROTAGONIST ,BATHYSPHERE>
		<TELL "   The bathysphere ">)>
	 <COND (<EQUAL? ,BATHYSPHERE-DEPTH 0>
		<DEQUEUE I-BATHYSPHERE>
		<SETG BATHYSPHERE-DIRECTION 0>
		<MOVE ,BATHYSPHERE ,HOLD>
	 	<MOVE ,WALDO ,HOLD>
		<COND (<IN? ,PROTAGONIST ,BATHYSPHERE>
		      ...
		      (T
		       <RFALSE>)>)
	       (<EQUAL? ,BATHYSPHERE-DEPTH 4>
		<DEQUEUE I-BATHYSPHERE>
		<SETG BATHYSPHERE-DIRECTION 0>
		<MOVE ,BATHYSPHERE ,LAKE-BOTTOM>
		...
		<COND (<EQUAL? ,BATHYSPHERE-DIRECTION -1>
		      ...
		      (<EQUAL? ,BATHYSPHERE-DIRECTION 1>
		       <TELL "a">)>
		<TELL "scend." CR>
		<COND (<G? ,CHESS-PIECE-DROWNED 0>
		       <COND (<PROB ,CHESS-PIECE-DROWNED>
...
beam of the exterior light." CR>)
			     (T
			      <SETG CHESS-PIECE-DROWNED
				    <+ ,CHESS-PIECE-DROWNED 10>>)>)>)>
	 <RTRUE>>
...
cardinal directions.")
      ...
      (VALUE 18)
      (MAP-LOC <PTABLE LAKE-AREA-MAP-NUM MAP-GEN-Y-5 MAP-GEN-X-4>)
...
clearer.")
      ...
      (FLAGS RLANDBIT UNDERGROUNDBIT)
      (GLOBAL LAKE-FLATHEAD)
      (MAP-LOC <PTABLE LAKE-AREA-MAP-NUM MAP-GEN-Y-6 MAP-GEN-X-1>)
...
Beautiful freshwater fish swim among slowly waving spenseweeds.")
      ...
      (FLAGS RLANDBIT UNDERGROUNDBIT)
      (GLOBAL LAKE-FLATHEAD)
      (MAP-LOC <PTABLE LAKE-AREA-MAP-NUM MAP-GEN-Y-7 MAP-GEN-X-1>)
      (ICON LAKE-BOTTOM-ICON)
      (ACTION LAKE-BOTTOM-F)>

<ROUTINE LAKE-BOTTOM-F (RARG)
...
and south.")
      ...
      (FLAGS RLANDBIT ONBIT UNDERGROUNDBIT)
      (MAP-LOC <PTABLE LAKE-AREA-MAP-NUM MAP-GEN-Y-5 MAP-GEN-X-6>)
      (ICON BASE-OF-MT-ICON)>

<ROUTINE G-U-MOUNTAIN-ENTER-F ()
...
"The stalls here once held thousands of royal mounts. The only exit is south.")
      ...
      (FLAGS RLANDBIT UNDERGROUNDBIT)
      (SYNONYM STABLE)
      (MAP-LOC <PTABLE LAKE-AREA-MAP-NUM MAP-GEN-Y-4 MAP-GEN-X-6>)
      (ICON STABLE-ICON)
      (ACTION STABLE-F)>
...
"The rooster stops moving and takes on the complexion of wrought iron.")
	(ANIMATE-ROUTINE I-WEATHER-VANE-TO-ROOSTER)
	(SIZE 10)
	(ACTION ROOSTER-F)>
...
<ROUTINE I-WEATHER-VANE-TO-ROOSTER ()
	 <FSET ,ROOSTER ,ANIMATEDBIT>
	 <COND (<VISIBLE? ,ROOSTER>
		<TELL
"   The air is split by a loud \"Cock-a-doodle-doo!\" as the weather
vane once again becomes a rooster." CR>)
	       (T
		<RFALSE>)>>
...
<ROOM G-U-MOUNTAIN
      ...
      (FLAGS RLANDBIT ONBIT UNDERGROUNDBIT)
      (GLOBAL LAKE-FLATHEAD)
      (MAP-LOC <PTABLE LAKE-AREA-MAP-NUM MAP-GEN-Y-5 MAP-GEN-X-7>)
      (ICON G-U-MOUNTAIN-ICON)
      (ACTION G-U-MOUNTAIN-F)>

<ROUTINE G-U-MOUNTAIN-F (RARG)
	 <COND (<AND <EQUAL? .RARG ,M-ENTER>
...
northeast, and a steep gravelly passage leads downward at an alarming angle.")
	...
	(FLAGS RLANDBIT UNDERGROUNDBIT)
	(SYNONYM GROTTO)
	(MAP-LOC <PTABLE LAKE-AREA-MAP-NUM MAP-GEN-Y-3 MAP-GEN-X-7>)>
...
<ROUTINE LOWEST-HALL-ENTER-F ("AUX" (CURRENT-GRAVEL <>) (SPILL <>))
	 <TELL
...
<ROUTINE GENERIC-GRAVEL-F (TBL LEN)
...
<ROUTINE GRAVEL-F ()
	 <COND (<VERB? EXAMINE MEASURE COUNT>
...
<ROOM SHRINE
      ...
      (REGION "Region:  Unknown")
      (LDESC
"You have stumbled upon the long-hidden shrine to Saint Foobus of Galepath.
An idol of Foobus is carved from the very rock that forms this cave. The only
exit is southwest.")
      (SW TO GROTTO)
      (OUT TO GROTTO)
      (FLAGS RLANDBIT UNDERGROUNDBIT)
      (MAP-LOC <PTABLE LAKE-AREA-MAP-NUM MAP-GEN-Y-2 MAP-GEN-X-8>)
      (ICON SHRINE-ICON)>

<OBJECT IDOL
	(LOC SHRINE)
	(DESC "idol")
	(SYNONYM SAINT IDOL FOOBUS GALEPATH SHRINE)
	(ADJECTIVE SAINT ST)
	(FLAGS NDESCBIT VOWELBIT)
	(RESEARCH
...
of the elixir. It is now ">
		      ...
		      (<PRSO? ,DRINKING-STRAW>
		       <MOVE ,DRINKING-STRAW ,BOWL>
		       <TELL
"The straw extends just above the rim of the bowl." CR>)
		      (<PRSO? ,RING ,ZORKMID-COIN ,EAST-KEY ,WEST-KEY
			      ,RUSTY-KEY ,SQUID-REPELLENT ,UNOPENED-WALNUT
			      ,WALNUT-SHELL ,WALNUT ,RUBY ,SAPPHIRE>
		       <MOVE ,PRSO ,BOWL>
		       <FCLEAR ,PRSO ,TAKEBIT>
		       <TELL
...
clear how you'll ever get it out again..." CR>)
	       ...
	       (<AND <VERB? TAKE MOVE TIP TIP-OVER>
		     <PRSO? ,BOWL>>
		<TELL
"The elixir's at the bottom of a bowl which is
...
<ROUTINE ELIXIR-F ()
	 <COND (<VERB? TOUCH REACH-IN>
		...
		<RTRUE>)
	       (<VERB? DRINK TASTE>
		<COND (<IN? ,DRINKING-STRAW ,BOWL>
		       <PERFORM ,V?DRINK-WITH ,ELIXIR ,DRINKING-STRAW>
		       <RTRUE>)
		      (T
		       <PERFORM ,V?TAKE ,BOWL>
		       <RTRUE>)>)
...
there's little chance you'd be able to climb back.")
      ...
      (SYNONYM LAIR)
      (ADJECTIVE BAT\'S)
      (MAP-LOC <PTABLE LAKE-AREA-MAP-NUM MAP-GEN-Y-6 MAP-GEN-X-7>)
      (ICON BATS-LAIR-ICON)>
...
Trails wind north and southwest among the trees.")
      ...
      (SW TO G-U-SAVANNAH)
      (FLAGS RLANDBIT ONBIT UNDERGROUNDBIT)
      (MAP-LOC <PTABLE LAKE-AREA-MAP-NUM MAP-GEN-Y-6 MAP-GEN-X-6>)
      (ACTION G-U-WOODS-F)>

<ROUTINE G-U-WOODS-F (RARG)
	 <COND (<AND <EQUAL? .RARG ,M-ENTER>
...
<OBJECT FOX
	...
	(ANIMATE-ROUTINE I-STOLE-TO-FOX)
	(SIZE 15)
	(ACTION FOX-F)>
...
<ROUTINE I-STOLE-TO-FOX ()
	 <FSET ,FOX ,ANIMATEDBIT>
	 <FCLEAR ,FOX ,WEARBIT>
	 <FCLEAR ,FOX ,WORNBIT>
	 <COND (<VISIBLE? ,FOX>
		<TELL "   The fox suddenly shakes its bushy tail." CR>)
	       (T
		<RFALSE>)>>
...
at a desert to the west. A herd of unicorns is grazing nearby.")
      ...
      (FLAGS RLANDBIT ONBIT UNDERGROUNDBIT)
      (MAP-LOC <PTABLE LAKE-AREA-MAP-NUM MAP-GEN-Y-7 MAP-GEN-X-5>)
...
that he is holding the framed document.")
      (MAP-LOC <PTABLE LAKE-AREA-MAP-NUM MAP-GEN-Y-6 MAP-GEN-X-3>)
...
<ROUTINE CAMEL-DRINK-ROOM-F (RARG)
	 <COND (<EQUAL? .RARG ,M-LOOK>
...
other cardinal directions.")
      ...
      (MAP-LOC <PTABLE LAKE-AREA-MAP-NUM MAP-GEN-Y-7 MAP-GEN-X-4>)
...
<ROOM RING-OF-DUNES
      ...
      (NW SORRY "Sand dunes block your way.")
      (FLAGS RLANDBIT ONBIT DESERTBIT UNDERGROUNDBIT)
      (MAP-LOC <PTABLE LAKE-AREA-MAP-NUM MAP-GEN-Y-7 MAP-GEN-X-3>)
...
grows gaudier, and a few bars of honky-tonk music drift through the air.")
	(ANIMATE-ROUTINE I-FIXTURE-TO-CAMEL)
	(ACTION CAMEL-F)>
...
<ROUTINE I-FIXTURE-TO-CAMEL ()
	 <COND (,TIME-STOPPED
		<QUEUE I-FIXTURE-TO-CAMEL 3>
		<RFALSE>)>
	 <FSET ,CAMEL ,ANIMATEDBIT>
	 <FSET ,CAMEL ,ACTORBIT>
	 <COND (<VISIBLE? ,CAMEL>
...
<ROUTINE DESERT-ROOM-F (RARG)
	 <COND (<AND <EQUAL? .RARG ,M-ENTER>
		     <NOT <IN? ,PROTAGONIST ,CAMEL>>>
		<SETG DESERT-DEATH <+ ,DESERT-DEATH 1>>
		<QUEUE I-DESERT-RESET -1>
		<TELL
"You trudge along beneath the searing gaze of an
artificial desert sun" ,ELLIPSIS>)
	       (<AND <EQUAL? .RARG ,M-END>
...
<ROOM FROBOZZ-PHILHARMONIC-HALL
      (LOC ROOMS)
      (DESC "Frobozz Philharmonic Hall")
      (REGION "Flatheadia")
      (NORTH PER FROBOZZCO-BASEMENT-ENTER-F)
      ...
      (MAP-LOC <PTABLE LAKE-AREA-MAP-NUM MAP-GEN-Y-3 MAP-GEN-X-4>)
      (ACTION FROBOZZ-PHILHARMONIC-HALL-F)
      (THINGS LONE SPOTLIGHT SPOTLIGHT-PSEUDO)>

<ROUTINE FROBOZZ-PHILHARMONIC-HALL-F (RARG)
	 <COND (<EQUAL? .RARG ,M-LOOK>
		<TELL
...
"You are in a tiny space beneath the stage. There are no visible exits.")
      ...
      (SYNONYM PIT)
      (ADJECTIVE CONDUCTOR)
      (MAP-LOC <PTABLE LAKE-AREA-MAP-NUM MAP-GEN-Y-3 MAP-GEN-X-3>)
      (ACTION CONDUCTOR-PIT-F)>

<ROUTINE CONDUCTOR-PIT-F (RARG)
	 <COND (<AND <EQUAL? .RARG ,M-ENTER>
...
in the front of the theatre. Exits lead west and southwest.")
      (WEST TO FROBOZZ-PHILHARMONIC-HALL)
      (SW TO NORTH-SHORE)
      (FLAGS RLANDBIT ONBIT)
      (SYNONYM THEATRE THEATER)
      (MAP-LOC <PTABLE LAKE-AREA-MAP-NUM MAP-GEN-Y-3 MAP-GEN-X-5>)
      (THINGS DIMWIT\'S BOX DIMWIT-BOX-PSEUDO)>

<ROUTINE DIMWIT-BOX-PSEUDO ()
	 <COND (<VERB? EXAMINE>
