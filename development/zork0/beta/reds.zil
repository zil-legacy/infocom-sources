...
<ENTRY	RED-O-ADJ RED-O-PP RED-O-NP>
...
<DEFINE REDUCE-EXCEPT-IT (PHR NP)
	<COND (<AND <==? 1 <NOUN-PHRASE-COUNT .NOUN-PHRASE>>
		    <==? ,IT <NOUN-PHRASE-OBJ1 .NOUN-PHRASE>>>
	       <COND (<ZERO? ,P-IT-OBJECT>
		      <PARSER-ERROR 0 ,PARSER-ERROR-NOOBJ .NP>)
		     ;(<NOT <VISIBLE? ,P-IT-OBJECT>>
		      <NOT-HERE ,P-IT-OBJECT>)
		     (T
		      <NOUN-PHRASE-OBJ1 .NOUN-PHRASE ,P-IT-OBJECT>)>)>>
...
<DEFINE DETERMINE-NP DN (SEARCH-ACT:<OR FIX TABLE> WHICH:FIX OBJ:PMEM
  ...
  ;"<FIND-QUANT ,FINDER .QUANT>
  <FIND-ADJS ,FINDER <NP-ADJS .ROBJ>>
  <FIND-NOUN ,FINDER <NP-NAME .ROBJ>>
  <FIND-OF ,FINDER <NP-OF .ROBJ>>
  <FIND-EXCEPTIONS ,FINDER <NP-EXCEPT .ROBJ>>"
  <MAKE-FINDER	'FINDER ,FINDER
		'FIND-APPLIC .SEARCH-ACT
		'FIND-QUANT .QUANT
		'FIND-SYNTAX .SYN
...
<DEFINE RED-O-PP ("OPT" N:FIX TYP:FIX "AUX" PP A PREP)
 ...
 <COND (<AND <T? ,P-OFLAG>
	     <ZERO? <ZGET ,O-LEXV ,P-OFLAG>>
	     <SET A <ZGET ,O-LEXV <- ,P-OFLAG ,P-LEXELEN>>>
	     <SET PP <POP-PSTACK ,DATA-STACK>>
	     <OR <AND <==? .N 1> ;<PMEM-TYPE? .PP PP>
		      ;<EQUAL? .A <PP-PREP .PP>>>
		 <AND ;<PMEM-TYPE? .PP NP>
		      <SET PREP <POP-PSTACK ,DATA-STACK>> ;<EQUAL? .A >>>>
	<ZPUT ,O-LEXV ,P-OFLAG ,W?NO.WORD>
	<COPYT ,O-LEXV ,G-LEXV ,LEXV-LENGTH-BYTES>
	<COPYT ,O-INBUF ,G-INBUF <+ 1 ,INBUF-LENGTH>>
	<ZPUT ,OOPS-TABLE ,O-START <ZGET ,OOPS-TABLE ,O-AGAIN>>
	<INSERT-NP ;,P-OFLAG <COND (<EQUAL? .A .PREP> 1) (T 0)>>
	<SETG P-OFLAG <>>
	<COPY-INPUT T>
	<THROW ,PARSER-RESULT-AGAIN ,PARSE-SENTENCE-ACTIVATION>)>>

<DEFINE INSERT-NP ("OPT" (NUM 0) (NP <>) "AUX" (GPTR ,P-OFLAG) PPTR TMP)
...
<DEFINE RED-O-NP ("OPT" N:FIX TYP:FIX "AUX" A NP (PP <>))
 ...
 <COND (<T? ,P-OFLAG>
	...
	<COPYT ,O-LEXV ,G-LEXV ,LEXV-LENGTH-BYTES>
	<COPYT ,O-INBUF ,G-INBUF <+ 1 ,INBUF-LENGTH>>
	<COND (<ZERO? <SET A <ZGET ,G-LEXV ,P-OFLAG>>>	;"PARSER-ERROR-ORPH-S"
	       <ZPUT ,G-LEXV ,P-OFLAG ,W?NO.WORD>
	       <INSERT-NP 0 .NP>)
	      (T
	       <PROG ((A1 <>))
		<COND (<SET A <NP-NAME .NP>>
		       <ZPUT ,G-LEXV ,P-OFLAG .A>)>
		<COND (<SET A <NP-ADJS .NP>>
		       <INSERT-ADJS .A>)>
		<COND (<T? .PP>
		       ;<SET PP <NP-LOC .NP>>
		       <MAKE-ROOM-FOR-TOKENS 2 ,G-LEXV <+ ,P-OFLAG ,P-LEXELEN>>
		       <ZPUT ,G-LEXV <+ ,P-OFLAG ,P-LEXELEN>
				     <LOCATION-PREP .PP>>
		       ...
		       <COND (<PMEM-TYPE? .A NOUN-PHRASE>
			     ...
			     (<PMEM-TYPE? .A NPP>
			      <SET A <NPP-NOUN .A>>)>
		       <ZPUT ,G-LEXV <+ ,P-OFLAG <* 2 ,P-LEXELEN>>
				     <NP-NAME .A>>)>
...
<DEFINE HACK-TELL ACT (X "AUX" NP)
	...
	<COND (<NOT <EQUAL? .X ,WINNER ,PLAYER ,ME ,YOU>>
	       <IGNORE-FIRST-WORD ,W?YOU>
	       <COND (<L? ,P-LEN 1>
		      <SETG P-CONT <>>)
		     (T
		      <SETG P-CONT ,TLEXV>)>
	       <COND (<EQUAL? ,M-FATAL <HACK-TELL-1 .NP>>
		      <SETG P-CONT -1>
...
<DEFINE RED-NP ("OPT" N:FIX TYP:FIX
  ...
  <COND (<EQUAL? .NAME 1>
	 <SET NAME <>>)>
  <COND (<EQUAL? ,W?COMMA <ZGET .LEXE 0>>
...
<DEFINE RED-ADJ RA ("OPT" N:FIX TYP:FIX "AUX" A1 A2 CT AD OBJ)
  <COND (<NOT <ASSIGNED? N>>
...
<DEFINE RED-QUOTE ACT ("OPT" N:FIX TYP:FIX "AUX" NP)
 ...
 <COND (<EQUAL? ,W?QUOTE <POP-PSTACK ,DATA-STACK>>
	...
	<SET NP <PMEM-ALLOC NP NAME ,W?QUOTE
			    LEXBEG <ZBACK ,P-RUNNING <* 2 <* 2 ,P-LEXELEN>>>>>
	<REPEAT ()
	 <COND (<OR <L? <SETG P-LEN <- ,P-LEN 1>> 0>
		    <EQUAL? <SET N <ZGET ,P-RUNNING 0>>
			    ,W?QUOTE ,W?END.OF.INPUT>>
		<NP-LEXEND .NP ,P-RUNNING>
		<SETG P-RUNNING <ZREST ,P-RUNNING <* 2 ,P-LEXELEN>>>
		<COND (<NOT <EQUAL? .N ,W?QUOTE>>
