...
<ROUTINE PLAIN-F (RARG "AUX" PIECE)
	 <COND (<EQUAL? .RARG ,M-LOOK>
		<COND (,DEBUG
		       <TELL
"[RANK: " N ,RANK ", FILE: " N ,FILE ", PLAIN-LOC: " N ,PLAIN-LOC ".]" CR>)>
...
<ROUTINE PLAIN-MOVEMENT-F ()
	 <COND (<OR <AND <EQUAL? ,RANK 1>
	 ...
	 <COND (<PRSO? ,P?WEST ,P?SW ,P?NW>
		<SETG FILE <- ,FILE 1>>)>
	 <PUT-ALL-IN-STORAGE ,PLAIN-OFFSET ,PLAIN-LOC>
	 <SETG PLAIN-LOC <- <+ <* <- ,RANK 1> 8> ,FILE> 1>>
	 <GET-ALL-FROM-STORAGE ,PLAIN-OFFSET ,PLAIN-LOC>
	 ,PLAIN>
...
<OBJECT BLACK-QUEEN
	(DESC "queen")
	(LDESC
"A regal woman proudly surveys the landscapes in all directions. Her skin
...
crenellations of the parapet you spot a man, dressed in a ivory chain
...
His tall, ebony headpiece bears a religious cipher, and his robes seem to soak
up all light.")
...
<OBJECT WHITE-QUEEN
	(DESC "queen")
	(LDESC
"A regal woman proudly surveys the landscapes in all directions. Her royal
...
<ROUTINE CHESS-PIECE-F ("OPT" (ARG <>) "AUX" CNT)
...
"\"The terrain is strange and unfamiliar; I am too terrified to move!\"" CR>)
		      ...
		      (T
		       ...
		       <PUT ,CHESS-PIECE-MOVE-TABLE ,DIR-CNT ,PRSO>
		       ...
		       <COND (<AND <G? ,DIR-CNT 1>
						...
						,BLACK-KNIGHT>>
				   <NOT <EQUAL? ,PRSO
						<GET ,CHESS-PIECE-MOVE-TABLE
						     <- ,DIR-CNT 2>>>>>
			      ;"checks to make sure that all directions given
				are the same, except in the case of knights"
			      <SETG DIR-CNT 0>
			      <COPYT ,CHESS-PIECE-MOVE-TABLE 0 16>
			      <TELL ,CANNOT-TRAVEL>)
			     (<OR ,P-CONT ,M-PTR>
			      <COND (,DEBUG
				     <TELL "[DIR-CNT is " N ,DIR-CNT ".]" CR>)>
			      <SETG CLOCK-WAIT T>)
			     (T
			      <SETG DIR-CNT 0>
			      <MOVE-CHESS-PIECE>)>)>
...
"Perhaps you should tell" T ,PRSO " the direction(s)." CR>)
		...
		<COND (,TIME-STOPPED
		       <PERFORM ,V?TELL ,PRSI>
		       <RTRUE>)
		      (<AND <ULTIMATELY-IN? ,SMALL-VIAL ,PRSO>
			    <FSET? ,SMALL-VIAL ,TRYTAKEBIT>>
		       <RTRUE>)>
		<MOVE ,PRSO ,PRSI>
		<TELL "The " D ,PRSI " takes" T ,PRSO ".">
		<COND (<AND <PRSO? ,PIGEON>
			    <NOT <EQUAL? ,HERE <META-LOC ,PERCH>>>>
		       <CHESS-PIECE-TAKES-PERCH ,PRSI>)
		      (T
		       <TELL
...
<ROUTINE CHESS-PIECE-TAKES-PERCH (PIECE "OPTIONAL" (DO-CR T))
	 <MOVE-TO-PERCH .PIECE>
	 <TELL " Instantly,">
...
<CONSTANT CHESS-PIECE-MOVE-TABLE
	<TABLE 0 0 0 0 0 0 0 0>>

<ROUTINE MOVE-CHESS-PIECE
	 ...
	 <SET CNT 0>
	 <REPEAT ()
		 <SET DIR <GET ,CHESS-PIECE-MOVE-TABLE .CNT>>
		 <COND (<EQUAL? .DIR <>>
			<RETURN>)>
		 ...
		 <COND (<AND <EQUAL? ,HERE ,CONSTRUCTION>
			     ...
			     <CONSTRUCTION-OBSTRUCTION .NEW-LOC .DIR>>
			<SET BLOCK T>
...
"\"My word! There appears to be a wall in the way!\"" CR>)>
			<RETURN>)>
 		 <SET NEW-LOC <+ <* <- .NEW-RANK 1> 8> <- .NEW-FILE 1>>>
		 <COND (<AND <GET ,CHESS-PIECE-MOVE-TABLE .CNT>
			     <NOT <EQUAL? ,WINNER ,BLACK-KNIGHT ,WHITE-KNIGHT>>
...
"\"Alas, the path between here and there is not unobstructed.\"" CR>
			<RETURN>)>>
	 <SET DIR <DIR-TO-STRING <GET ,CHESS-PIECE-MOVE-TABLE 0>>>
	 ...
	 <COND (,DEBUG
		<TELL
"[NEW-FILE is " N .NEW-FILE ". NEW-RANK is " N .NEW-RANK ". NEW-LOC is "
N .NEW-LOC ".]" CR>)>
...
a proud whinny in the distance.">)
		...
		<CHESS-PIECE-SNARF <+ .NEW-LOC .OFFSET> ,WINNER>
...
<ROUTINE CHESS-PIECE-SNARF (NEW-LOC SNARFER "AUX" OBJ (CNT 0))
	 <COND (,DEBUG
		<TELL
"[Calling CHESS-PIECE-SNARF. NEW-LOC = " N .NEW-LOC ".]" CR>)>
	 <REPEAT ()
...
<ROUTINE CONSTRUCTION-OBSTRUCTION
	 		(L DIR "OPT" (CALLED-BY-EXIT-F <>) "AUX" (CHANGE 0))
...
<ROUTINE CONSTRUCTION-F (RARG "AUX" (CNT 0))
	 <COND (<EQUAL? .RARG ,M-LOOK>
		...
		<COND (,DEBUG
		       <TELL
"[RANK is " N ,RANK ", FILE is " N ,FILE
", and CONSTRUCTION-LOC is " N ,CONSTRUCTION-LOC ".]" CR>)>
		<TELL
...
"Engraved on the wall is the number " N ,CONSTRUCTION-LOC ".">)
		...
		<GET-ALL-FROM-STORAGE ,CONSTRUCTION-OFFSET
				      ,CONSTRUCTION-LOC>)>>
...
<ROUTINE CONSTRUCTION-MOVEMENT-F ("AUX" (CHANGE 0))
	 <PUT-ALL-IN-STORAGE ,CONSTRUCTION-OFFSET ,CONSTRUCTION-LOC>
	 <SET CHANGE <CONSTRUCTION-OBSTRUCTION ,CONSTRUCTION-LOC ,PRSO T>>
	 <COND (<EQUAL? .CHANGE 100>
		<RETURN ,FIELD-OFFICE>)>
	 <SETG RANK <+ </ ,CONSTRUCTION-LOC 8> 1>>
	 <SETG FILE <+ <MOD ,CONSTRUCTION-LOC 8> 1>>
	 <COND (<EQUAL? .CHANGE 0>
		<GET-ALL-FROM-STORAGE ,CONSTRUCTION-OFFSET ,CONSTRUCTION-LOC>
		<CANT-GO>
		<RFALSE>)
	       (T
...
<ROUTINE REMOVE-ANY-OTHER-CHESS-PIECE-THERE (L TAKER "AUX" TAKEE (CNT 0))
	 <COND (,DEBUG
		<TELL
"[Calling REMOVE-ANY-OTHER-CHESS-PIECE-THERE. LOC = " N .L " and TAKER = "
D .TAKER ".]" CR>)>
	 <REPEAT ()
		 <COND (<NOT <L? .CNT ,STORAGE-TABLE-LENGTH>>
			<RETURN>)
...
<ROUTINE PUT-IN-STORAGE (OFFSET OBJ L "AUX" (CNT 0))
	 <REPEAT ()
		 <COND (<EQUAL? <GET ,STORAGE-TABLE .CNT> 0>
			<COND (,DEBUG
			       <TELL "[Putting " T .OBJ " in storage with a ">
			       <COND (<EQUAL? .OFFSET ,PLAIN-OFFSET>
				      <TELL "Plain">)
				     (<EQUAL? .OFFSET ,CONSTRUCTION-OFFSET>
				      <TELL "Construction">)
				     (T
				      <TELL "FrobozzCo">)>
			       <TELL
" value. CNT is " N .CNT " and LOC is " N .L ".]" CR>)>
...
<ROUTINE PUT-ALL-IN-STORAGE (OFFSET L "OPT" (RM ,HERE) "AUX" (CNT 0) F N)
	 <SET F <FIRST? .RM>>
	 <REPEAT ()
	       <COND (.F
		      <SET N <NEXT? .F>>)
...
<ROUTINE GET-ALL-FROM-STORAGE (OFFSET L "OPT" (RM ,HERE) "AUX" (CNT 0))
	 <REPEAT ()
		 <COND (<NOT <L? .CNT ,STORAGE-TABLE-LENGTH>>
			<RETURN>)
