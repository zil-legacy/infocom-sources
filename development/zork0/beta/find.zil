...
<DEFINE FIND-OBJECTS ("OPT" (SEARCH:FIX
  ...
  <FIND-RES-NEXT .RES <>>
  ;"Initialize world"
  <COND (<AND .PARENT <NOT <IN? .PARENT ,GLOBAL-OBJECTS>>>
	 ;"In case we have `the foo in the bar' but not `a fly on the wall'"
	 <SET CONT? <>>
	 <FIND-DESCENDANTS .PARENT <ORB ,FD-INCLUDE? ,FD-SEARCH? ,FD-NEST?>;7>)
	(T
	 ...
	 <PROG ((LOSING? <>))
	   ...
	   <COND (<NOT <0? <FIND-RES-COUNT .RES>>>
		 ...
		 (<AND <NOT <BTST .SEARCH ,SEARCH-ALL>>
		       <NOT .LOSING?>>
		  <COND (<OR <T? <WORD-CLASSIFICATION-NUMBER
					<SET GLBS <LEXV-WORD ,TLEXV>>>>
...
;<DEFINE MATCH-OF-OBJECTS (OVEC "AUX" NP ADJS NOUN)
	<SET NP <FIND-OF ,FINDER>>	;"See if OF-phrase matches too."
	<SET ADJS <FIND-ADJS ,FINDER>>
	<SET NOUN <FIND-NOUN ,FINDER>>
	<FIND-ADJS ,FINDER <NP-ADJS .NP>>
	<FIND-NOUN ,FINDER <NP-NAME .NP>>
	<REPEAT ((CT <FIND-RES-COUNT .OVEC>) (OCT .CT)
		 (VEC <REST-TO-SLOT .OVEC FIND-RES-OBJ1>))
		<COND (<DLESS? CT 0>
		       <RETURN>)
		      (<MATCH-OBJECT <ZGET .VEC 0> ,FINDER T>
		       <DEC OCT>
		       <FIND-RES-COUNT .OVEC .OCT>
		       <COPYT <ZREST .VEC 2> .VEC <* 2 .CT>>)
		      (T
		       <SET VEC <ZREST .VEC 2>>)>>
	<FIND-ADJS ,FINDER .ADJS>
	<FIND-NOUN ,FINDER .NOUN>>
